...
#---Script Start---#
echo "*************************************************************************"
echo "$DateTime - Munki 3 postinstall v${script_version}"
...
# This is useful for loading launch daemons and agents.
    ...
    for pid_uid in $(ps -axo pid,uid,args | grep -i "[l]oginwindow.app" | awk '{print $1 "," $2}'); do
        ...
        launchctl asuser "$uid" chroot -u "$uid" / launchctl unload /Library/LaunchAgents/com.googlecode.munki*
        launchctl asuser "$uid" chroot -u "$uid" / launchctl load -w /Library/LaunchAgents/com.googlecode.munki*
    done
